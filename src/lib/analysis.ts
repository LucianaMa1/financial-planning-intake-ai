...
function formatCurrencyLike(value: unknown) {
  ...
  if (!Number.isFinite(amount)) return stringifyValue(value);
  return `$${amount.toLocaleString()}`;
}

function normalizeHouseholdSummary(value: unknown, fallback: string) {
  if (typeof value === "string") return value;
  ...
  const clients = Array.isArray(source.clients)
    ? source.clients
        .map((item) => {
          ...
          const name = stringifyValue(client.name);
          const occupation = stringifyValue(client.occupation);
          const income = client.annualIncome ?? client.income;
          const parts = [name, occupation ? `(${occupation})` : "", income ? `earning about ${formatCurrencyLike(income)}` : ""].filter(Boolean);
          return parts.join(" ");
        })
        .filter(Boolean)
    : [];
  const dependents = Array.isArray(source.dependents) ? source.dependents.length : 0;
  const totalAnnualIncome = source.totalAnnualIncome ? formatCurrencyLike(source.totalAnnualIncome) : "";
  const estateStatus = stringifyValue(source.estatePlanningStatus);
  const insurance = source.insuranceSummary && typeof source.insuranceSummary === "object"
    ? source.insuranceSummary as Record<string, unknown>
    : null;
  const debt = source.liabilitiesSummary && typeof source.liabilitiesSummary === "object"
    ? source.liabilitiesSummary as Record<string, unknown>
    : null;
  const insuranceGap = insurance && stringifyValue(insurance.disability).toLowerCase().includes("no")
    ? "disability coverage appears absent"
    : "";
  const debtLoad = debt && debt.mortgageBalance ? `reported mortgage balance is about ${formatCurrencyLike(debt.mortgageBalance)}` : "";

  const sentences = [
    clients.length ? `${clients.join(" and ")} make up the reporting household.` : "",
    dependents ? `The intake lists ${dependents} dependents.` : "",
    totalAnnualIncome ? `Combined earned income is about ${totalAnnualIncome}.` : "",
    debtLoad ? `${debtLoad}.` : "",
    insuranceGap ? `A key protection issue is that ${insuranceGap}.` : "",
    estateStatus ? `Estate planning status: ${estateStatus}.` : "",
  ].filter(Boolean);
...
export async function generateAnalysis(data: QuestionnaireData): Promise<AnalysisResult> {
  ...
  try {
    ...
    return {
      source: "openai",
      generatedAt: new Date().toISOString(),
      householdSummary: normalizeHouseholdSummary(parsed.householdSummary, fallback.householdSummary),
